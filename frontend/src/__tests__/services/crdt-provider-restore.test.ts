...
/** Build the byte payload for a MSG_RESTORE_NOTIFICATION message. */
...
const VALID_RESTORE_PAYLOAD: RestoreNotificationPayload = {
  ...
  pre_commit_sha: "sha-pre",
  your_dirty_heading_paths: [["Section1"]],
};

describe("CrdtProvider restore notification handling", () => {
  it("notification-before-sync ordering: onRestoreNotification fires on SYNC_STEP_2 receipt", () => {
    const onRestore = vi.fn();
    const onSynced = vi.fn();
    const doc = new Y.Doc();
    const provider = new CrdtProvider(doc, "/test/doc.md", { onSynced, onRestoreNotification: onRestore });

    provider.connect();
    const ws = StubWebSocket.lastInstance!;
    ws.open();

    // Server sends MSG_RESTORE_NOTIFICATION first, then MSG_SYNC_STEP_2
  ...
  it("pendingRestoreNotification is reset on reconnect", () => {
    const onRestore = vi.fn();
    const doc = new Y.Doc();
    const provider = new CrdtProvider(doc, "/test/doc.md", { onRestoreNotification: onRestore });

    provider.connect();
    const ws1 = StubWebSocket.lastInstance!;
    ws1.open();

    // Receive a notification on the first connection (but no SYNC_STEP_2 yet)
    ...
    // Send SYNC_STEP_2 on the NEW connection — onRestoreNotification should NOT fire
    // because pendingRestoreNotification was cleared in onopen.
    const sourceDoc = new Y.Doc();
  ...
  it("MSG_RESTORE_NOTIFICATION with malformed JSON closes the socket", () => {
    const onError = vi.fn();
    const doc = new Y.Doc();
    ...
    // Send a MSG_RESTORE_NOTIFICATION with invalid JSON in the payload

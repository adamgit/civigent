...
Read several sections from the published/live wiki in one call — a nested section "Weapons" inside "Ship Building", and the content before the first heading:
```
{{tool:readPublishedSection}}(sections: [
  { doc_path: "/my-doc.md", heading_path: ["Ship Building", "Weapons"] },
  { doc_path: "/my-doc.md", heading_path: [] }
])
```

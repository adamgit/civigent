...
import { getSnapshotHealth, getSnapshotHistory, snapshotAllDocs, SnapshotGenerationDisabledError, SnapshotRootNotWritableError } from "../../storage/snapshot.js";
import {
...
export function createApiRouter(options?: CreateApiRouterOptions): express.Router {
  ...
  router.get("/documents/:docPath(*)/diagnostics", async (req, res, next) => {
    try {
      ...
      res.json(await buildDocumentDiagnostics(docPath));
    } catch (error) {
      next(error);
    }
  });

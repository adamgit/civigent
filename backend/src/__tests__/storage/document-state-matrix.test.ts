...
/**
 * U5: every proposal-overlay layer must carry a claimed-section-keys provider —
 * there is no wholesale fallback. These lower-level mechanics tests claim NOTHING
 * structurally (an empty manifest), which is the honest representation: the overlay
 * owns only what its own skeleton materializes; everything else is inherited from
 * canonical. (The one scenario that needs an explicit delete-claim wires its own.)
 */
function shadow(overlay: string, canonical: string): ProposalShadowContentLayer {
  return new ProposalShadowContentLayer(overlay, canonical, async () => new Set<string>());
}
...
describe("Document state matrix", () => {
  ...
  describe("missing document", () => {
    ...
    it("getDocumentState returns 'missing'", async () => {
      const layer = shadow(overlayDir, canonicalDir);
  ...
  describe("live-empty document", () => {
    ...
    beforeAll(async () => {
      const layer = shadow(canonicalDir, canonicalDir);
  ...
  describe("live-non-empty document", () => {
    ...
    it("tombstoneDocument succeeds → tombstoned", async () => {
      ...
      const canonical = shadow(canonicalDir, canonicalDir);
      ...
      const overlay = shadow(overlayDir, canonicalDir);
...
describe("Document state edge cases", () => {
  ...
  it("live-empty → add BFH → delete BFH → still live-empty", async () => {
    ...
    const layer = shadow(ctx.contentDir, ctx.contentDir);
  ...
  it("tombstone with stale skeleton — tombstone takes precedence", async () => {
    ...
    const canonical = shadow(ctx.contentDir, ctx.contentDir);
    await canonical.createDocument(docPath);
    await canonical.upsertSection(
    ...
    const overlay = shadow(overlayDir, ctx.contentDir);
    await overlay.tombstoneDocument(docPath);
  ...
  it("overlay empty skeleton + deleted-id canonical section = live-empty (U5/D4)", async () => {
    ...
    // refuses because canonical exists). Under the U5 merge law, an empty overlay
    // skeleton ALONE does NOT empty the document — that would silently re-inherit
    // canonical. Emptying requires the proposal to record the canonical section's
    // section-file ID as deleted (identity-based delete detection, D4), so the merge
    // drops it by id.
    ...
    // With the deleted-id entry for "Sec"'s canonical section-file, the overlay is
    // live-empty.
    const canonSec = (await canonical.getSectionList(docPath)).find((e) => e.heading === "Sec")!;
    const overlay = new ProposalShadowContentLayer(
      overlayDir2,
      ctx.contentDir,
      async () => new Set<string>([canonSec.sectionFile]),
    );

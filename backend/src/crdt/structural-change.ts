 ...
 *
 * Heading-path uniqueness is persist's parse-and-map (see
 * `heading-addressability.ts`), not a classifier projection. This module
 * remains the sole shape detector for identity-preserving appliers and
 * proposal reflection. Ingress must not fork the classifier, mutate Y.Doc
 * or proposal state through it, or bleed quiescence-normalization
 * decisions into ingress rejection.
 */
